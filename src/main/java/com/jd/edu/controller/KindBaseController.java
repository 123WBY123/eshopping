...
@Api(value = "商品类别类")
@RestController
@RequestMapping("/edu/back/kindBase")
@CrossOrigin
public class KindBaseController {
    ...
    @ApiOperation("查询单个已被删除商品类别")
    @GetMapping("getOneDeleteBaseKind/{id}")
    public R getOneDeleteBaseKind(@PathVariable String id){
        KindBase kindBase = kindBaseService.getDeleteKindById(id);
        return R.ok().data("kindBase",kindBase);
    }

    @ApiOperation("修改已被删除商品类别")
    @PostMapping("updateDeleteKindBase")
    public R updateDeleteKindBase(@RequestBody KindBase kindBase){
        boolean result = kindBaseService.updateDeleteKind(kindBase);
        return result == true ? R.ok() : R.error();
    }
}
